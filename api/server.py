...
class RequestHandler(BaseHTTPRequestHandler):
    ...
    def send_anwser(self, which):
        asyncio.get_event_loop().run_until_complete(self.send_websocket_message(json.dumps({"req": "answer", "content": which} , sort_keys=True)))
        self.send_response(200 if api.__WEBSOCKET_ALIVE__  else 503)
        self.send_header("Server", "Dalvik\'s CTAS API Server")
        self.end_headers()

    def send_action(self, which):
        asyncio.get_event_loop().run_until_complete(self.send_websocket_message(json.dumps({"req": "action", "content": which} , sort_keys=True)))
        self.send_response(200 if api.__WEBSOCKET_ALIVE__  else 503)
        self.send_header("Server", "Dalvik\'s CTAS API Server")
        self.end_headers()
